...
| **P2** — reconnect backoff had no jitter, so every client orphaned by one outage waited the identical time and returned as a thundering herd. | *Equal* jitter — a pick from `[ceiling/2, ceiling)` — with the RNG injected the same way `open` and `wait` already are, so the tests still assert exact values instead of degrading to range checks. Full jitter was tried first and a pre-existing test caught it: `[0, ceiling)` has no lower bound and halves the expected total, so the wait before `isStalled` warns the reader sometimes fell under the five seconds that test guarantees. It failed about one run in eight — the kind of thing that gets waved through as flake. Equal jitter keeps dispersion across half the window while making the minimum total deterministic at 8000ms. |
